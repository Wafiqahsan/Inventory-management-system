...
def shower2():
    ...
    def filldata():
        ...
        cur=db.cursor()
        sql="select orderid,customerID,DateOfOrder,CategoryID,ProductID,QTY from orders"
        cur.execute(sql)
        data=cur.fetchall()
        global i
        for res in data:
    ...
    f=Entry(t,width=30)
    f.place(x=200,y=100)
    c=Label(t,text=' DateOfOrder')
    c.place(x=50,y=150)
    g=Entry(t,width=30)
    g.place(x=200,y=150)
    d=Label(t,text='CategoryID')
    d.place(x=50,y=200)
    h=Entry(t,width=30)
    h.place(x=200,y=200)
    j=Label(t,text='ProductID')
    j.place(x=50,y=250)
    la=Entry(t,width=30)
